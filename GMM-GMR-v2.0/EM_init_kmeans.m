function [Priors, Mu, Sigma, nbStates] = EM_init_kmeans(Data, nbStates)
%
...
[Data_id, Centers] = kmeans(Data', nbStates,'emptyaction','singleton'); 
            ...
            % Initially estimate number of clusters with mean-shift
        %sigma = estimate_bandwidth(f);
        %sigma = 60;
        %fprintf('global sigma = %g\n', sigma);
        
        % Cluster with Gonzalez85
% [Data_id, Centers] = gonzalez85_mex(Data, size(Data, 2), 60);
% Centers = Centers';
% Data_id = Data_id + 1;
% nbStates = size(Centers, 1)
Mu = Centers';
for i=1:nbStates
